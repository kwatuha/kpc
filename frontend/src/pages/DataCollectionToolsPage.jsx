...
const CATEGORIES = [
  ...
  // KEMRI / KIMES research M&E categories (seeded by
  // api/migrations/20260510_kemri_data_collection_templates.sql)
  { value: 'pi_visit',             label: 'KEMRI · PI site visit' },
  { value: 'cd_visit',             label: 'KEMRI · Centre Director visit' },
  { value: 'mel_audit',            label: 'KEMRI · MEL data quality audit' },
  { value: 'compliance',           label: 'KEMRI · SERU / Ethics compliance' },
  { value: 'adverse_event',        label: 'KEMRI · Adverse event / deviation' },
  { value: 'lab_qc',               label: 'KEMRI · Laboratory QC' },
  { value: 'equipment_audit',      label: 'KEMRI · Equipment audit' },
  { value: 'capacity_building',    label: 'KEMRI · Training & capacity building' },
  { value: 'stakeholder_feedback', label: 'KEMRI · Community engagement' },
  { value: 'study_closure',        label: 'KEMRI · Study close-out' },
];

// Categories created by the KEMRI seed; used by other pages to widen their
// "show this template here" filters so the KIMES library is reachable from
// project-level inspections, monitoring tabs, etc.
const KEMRI_TEMPLATE_CATEGORIES = new Set([
  'pi_visit',
  'cd_visit',
  'mel_audit',
  'compliance',
...
export { CATEGORIES, KEMRI_TEMPLATE_CATEGORIES };
...
export default function DataCollectionToolsPage() {
  ...
  const projectMetaById = useMemo(() => {
    ...
    for (const p of projects) {
      if (p?.id != null) {
        map.set(Number(p.id), {
          kimesProjectId: p.kimesProjectId || null,
          centreName: p.centreName || null,
        });
      }
    }
    return map;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        // Primary source: KEMRI research projects (KIMES). The data-collection
        // module is used to record monitoring visits, SERU audits, lab QC,
        // adverse events, community engagement, etc. against research studies.
        const rows = await kemriService.projects.listProjects({});
        const list = Array.isArray(rows) ? rows : [];
        const mapped = list.map((p) => ({
          id: p.id,
          // Compatibility shape used elsewhere in this file (projectsById,
          // Autocomplete getOptionLabel, draft restore, etc.).
          name: p.title || p.shortName || p.kimesProjectId || `Study #${p.id}`,
          projectName: p.title || p.shortName || p.kimesProjectId || `Study #${p.id}`,
          kimesProjectId: p.kimesProjectId || null,
          centreName: p.centreName || null,
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1100, mx: 'auto' }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Design KIMES checklists and forms (PI visits, SERU compliance, MEL data-quality audits, lab QC,
        adverse events, training feedback, community engagement, study close-out, etc.) and record
        monitoring visits against an active KEMRI research study.
      </Typography>
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>Research study</TableCell>
                        ...
                        {s.projectId != null ? (
                          <Stack spacing={0.25}>
                            <Typography variant="body2" fontWeight={600}>
                              {projectsById.get(Number(s.projectId)) || `Study #${s.projectId}`}
                            </Typography>
                            <Typography variant="caption" color="text.secondary">
                              {projectMetaById.get(Number(s.projectId))?.kimesProjectId ||
                                `ID ${s.projectId}`}
                              {projectMetaById.get(Number(s.projectId))?.centreName
                                ? ` · ${projectMetaById.get(Number(s.projectId)).centreName}`
                                : ''}
                            </Typography>
                          </Stack>
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <Autocomplete
              ...
              getOptionLabel={(p) => {
                ...
                const ref = p.kimesProjectId || `#${p.id}`;
              ...
              renderInput={(params) => (
                <TextField
                  {...params}
                  label="KEMRI research study"
                  required
                  helperText="Type to search by study title or KIMES project ID"
                />
              )}
            />
