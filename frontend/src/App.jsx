...
// KEMRI / KIMES pages — see api/adp/KEMRI_Concept.pdf for the 15-step workflow.
import KimesHomePage from './pages/KimesHomePage';
import KemriStudiesPage from './pages/KemriStudiesPage';
import KemriStudyRegistrationPage from './pages/KemriStudyRegistrationPage';
import PIDashboardPage from './pages/PIDashboardPage';
import PeerReviewQueuePage from './pages/PeerReviewQueuePage';
import PostStudyOutputsPage from './pages/PostStudyOutputsPage';
import KemriEscalationsInboxPage from './pages/KemriEscalationsInboxPage';
import KemriNotificationsInboxPage from './pages/KemriNotificationsInboxPage';
import KemriStudyFormExportPage from './pages/KemriStudyFormExportPage';
import KemriBoardScorecardPage from './pages/KemriBoardScorecardPage';
import KemriAiReportsPage from './pages/KemriAiReportsPage';
import KemriReportingCalendarPage from './pages/KemriReportingCalendarPage';
import KemriDonorPortalPage from './pages/KemriDonorPortalPage';
import KemriStrategicPlanPage from './pages/KemriStrategicPlanPage';
import KemriStrategicObjectivePage from './pages/KemriStrategicObjectivePage';
...
// KEMRI / KIMES dashboards (replace the legacy Machakos dashboards on the
// Operations / Finance / Summary Statistics routes).
import KemriExecutiveDashboardPage from './pages/KemriExecutiveDashboardPage';
import KemriOperationsDashboardPage from './pages/KemriOperationsDashboardPage';
import KemriFinanceDashboardPage from './pages/KemriFinanceDashboardPage';
...
const router = createBrowserRouter([
  ...
  { path: '/impes/*', element: <Navigate to="/" replace /> },
  {
    ...
    children: [
      {
        // KIMES landing replaces the legacy Machakos HomePage at "/".
        index: true,
        element: <KimesHomePage />,
      },
      {
        // Legacy Machakos county HomePage kept under /legacy-home for fallback.
        path: 'legacy-home',
        element: <HomePage />,
      },
      {
        // /dashboard now lands on the KEMRI Executive Summary so users hitting
        // the generic "Dashboard" link see KEMRI portfolio data, not the
        // legacy Machakos county landing.
        path: 'dashboard',
        element: <KemriExecutiveDashboardPage />,
      },
      {
        // Legacy Machakos role-aware dashboard landing kept for fallback.
      ...
      {
        path: 'kemri/studies',
        element: <KemriStudiesPage />,
      },
      {
        path: 'kemri/studies/new',
        element: <KemriStudyRegistrationPage />,
      },
      {
        path: 'kemri/studies/:id',
        element: <KemriStudyRegistrationPage />,
      },
      {
        path: 'kemri/dashboard/pi',
        element: <PIDashboardPage />,
      },
      {
        path: 'kemri/review-queue',
        element: <PeerReviewQueuePage />,
      },
      {
        path: 'kemri/outputs',
        element: <PostStudyOutputsPage />,
      },
      {
        // L1\u2013L4 non-conformity inbox + DG-NCF-001 letter generator
        path: 'kemri/escalations',
        element: <KemriEscalationsInboxPage />,
      },
      {
        // PI / CD / MEL / DG inbox \u2014 D-N reminders, escalation notices, SERU alerts
        path: 'kemri/notifications',
        element: <KemriNotificationsInboxPage />,
      },
      {
        // Filled "KEMRI Research Implementation & Grant Monitoring Form" v05
        // (mirrors api/adp/kemri_tools.pdf). Printable + .docx + .json download.
        path: 'kemri/studies/:id/form-export',
        element: <KemriStudyFormExportPage />,
      },
      {
        // Board Quarterly Scorecard (KIMES v5 §8.3)
        path: 'kemri/board-scorecard',
        element: <KemriBoardScorecardPage />,
      },
      {
        // AI Reporting Engine catalog (KIMES v5 §10.2)
        path: 'kemri/ai-reports',
        element: <KemriAiReportsPage />,
      },
      {
        // Concurrent Reporting Calendar (KIMES v5 §5)
        path: 'kemri/reporting-calendar',
        element: <KemriReportingCalendarPage />,
      },
      {
        // Donor Portal — Tier 1 read-only preview (KIMES v5 §5.1)
        path: 'kemri/donor-portal',
        element: <KemriDonorPortalPage />,
      },
      {
        // KEMRI Strategic Plan 2023-2027 alignment & key achievements (FINALAPPROVEDSTRATEGICPLAN3.27)
        path: 'kemri/strategic-plan',
        element: <KemriStrategicPlanPage />,
      },
      {
        // Drill-down into a single strategic objective (subprogram)
        path: 'kemri/strategic-plan/objective/:id',
        element: <KemriStrategicObjectivePage />,
      },
      {
        // KEMRI Executive Summary (replaces legacy Machakos SystemDashboardPage).
      ...
      {
        // KEMRI Operations dashboard (replaces legacy Machakos OperationsDashboardPage).
        path: 'operations-dashboard',
        element: <KemriOperationsDashboardPage />,
      },
      {
        // Legacy Machakos system dashboard kept under /legacy-system-dashboard for fallback.
        path: 'legacy-system-dashboard',
        element: <SystemDashboardPage />,
      },
      {
        // Legacy Machakos operations dashboard kept under /legacy-operations-dashboard for fallback.
        path: 'legacy-operations-dashboard',
      ...
      {
        // KEMRI Finance dashboard (replaces legacy Machakos FinanceDashboardPage).
        path: 'finance-dashboard',
        element: <KemriFinanceDashboardPage />,
      },
      {
        // Legacy Machakos finance dashboard kept under /legacy-finance-dashboard for fallback.
        path: 'legacy-finance-dashboard',
