...
export const ICON_MAP = {
  ...
  ScienceIcon: () => import('@mui/icons-material/Science').then(m => m.default),
  AddIcon: () => import('@mui/icons-material/Add').then(m => m.default),
  FolderOpenIcon: () => import('@mui/icons-material/FolderOpen').then(m => m.default),
  ChecklistIcon: () => import('@mui/icons-material/Checklist').then(m => m.default),
  PolicyIcon: () => import('@mui/icons-material/Policy').then(m => m.default),
  CalendarMonthIcon: () => import('@mui/icons-material/CalendarMonth').then(m => m.default),
  AutoAwesomeIcon: () => import('@mui/icons-material/AutoAwesome').then(m => m.default),
  AccountBalanceIcon: () => import('@mui/icons-material/AccountBalance').then(m => m.default),
};
...
export const getFilteredMenuCategories = (isAdmin = false, hasPrivilege = null, user = null) => {
  const categories = menuConfig.menuCategories.filter(category => {
    // Drop categories explicitly marked hidden (used to retire whole modules
    // that no longer apply to the KEMRI use case — e.g. county procurement).
  ...
  // Executive Viewer (DG / Board / donor stewardship): KIMES research portfolio
  // (read-only) + KEMRI executive / operations / finance / GIS dashboards.
  const allowedDashboardRoutes = [
    'SYSTEM_DASHBOARD',       // KEMRI Executive Summary
    'OPERATIONS_DASHBOARD',   // KEMRI Operations
    'FINANCE_DASHBOARD',      // KEMRI Finance
    'GIS_DASHBOARD',          // National GIS
  ];
  const allowedDashboardSet = new Set(allowedDashboardRoutes);

  const allowedKimesRoutes = [
    'KEMRI_HOME',
    'KEMRI_STUDIES',
    'KEMRI_PI_DASHBOARD',
    'KEMRI_ESCALATIONS',
    'KEMRI_OUTPUT_REGISTRY',
    'GIS_DASHBOARD',
  ];
  const allowedKimesSet = new Set(allowedKimesRoutes);

  return categories
    .filter((category) => category.id === 'kimes' || category.id === 'dashboard')
    .map((category) => {
      if (category.id === 'dashboard') {
        const filteredSubmenus = (category.submenus || [])
          .filter((submenu) => !submenu.hidden && allowedDashboardSet.has(submenu.route))
          .sort(
            (a, b) =>
      ...
      // KIMES — read-only research portfolio shortcuts
      const filteredSubmenus = (category.submenus || [])
        .filter((submenu) => !submenu.hidden && allowedKimesSet.has(submenu.route))
        .sort(
          (a, b) =>
            allowedKimesRoutes.indexOf(a.route) - allowedKimesRoutes.indexOf(b.route)
        );
      return { ...category, submenus: filteredSubmenus };
    })
