...
const kemriRoutes = require('./routes/kemriRoutes')
...
server.listen(port, bindHost, async () => {
    ...
    try {
        const { startScheduler: startKemriWorkflow } = require('./services/kemriWorkflowEngine');
        startKemriWorkflow({ everyMinutes: 360 });
        console.log('KEMRI workflow engine: started (D-30/-14/-7 reminders, L1\u2192L4 escalations, SERU alerts every 6h).');
    } catch (e) {
        console.warn('KEMRI workflow engine failed to start:', e.message);
    }
});
