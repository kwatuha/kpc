...
#
# Usage (standalone; KEMRI requires explicit target + flag):
#   export KEMRI_REMOTE_DEPLOY_ENABLED=yes
#   DEPLOY_HOST=... DEPLOY_USER=... DEPLOY_PATH=... DEPLOY_SYNC_DB_CONFIRM=yes ./deploy/sync-local-db-to-server.sh
...
#
# Optional: deploy/.env.deploy.db (not in git) can set DEPLOY_HOST DEPLOY_USER DEPLOY_PATH KEMRI_REMOTE_DEPLOY_ENABLED
#
set -euo pipefail
...
# KEMRI: no baked-in remote host — set DEPLOY_HOST, DEPLOY_USER, DEPLOY_PATH (or deploy/.env.deploy.db).
...
if [[ -f "$DEPLOY_DB_ENV" ]]; then
  load_env_file_allowlist "$DEPLOY_DB_ENV" "DEPLOY_HOST DEPLOY_USER DEPLOY_PATH KEMRI_REMOTE_DEPLOY_ENABLED DEPLOY_RESTORE_DOCKER_CONTAINER DEPLOY_PSQL_PATH DEPLOY_PSQL_DOCKER_IMAGE DEPLOY_SYNC_DB_CONFIRM DEPLOY_RESTORE_SUDO_POSTGRES DEPLOY_STRIP_VECTOR_EXTENSION_DDL"
fi

if [[ "$DRY_RUN" -eq 0 ]]; then
  if [[ "${KEMRI_REMOTE_DEPLOY_ENABLED:-}" != "yes" ]]; then
    echo "KEMRI: remote database sync is disabled by default (avoids pushing to the wrong server)." >&2
    echo "Set:  export KEMRI_REMOTE_DEPLOY_ENABLED=yes" >&2
    echo "And:  DEPLOY_HOST DEPLOY_USER DEPLOY_PATH plus DEPLOY_SYNC_DB_CONFIRM=yes" >&2
    exit 2
  fi
  if [[ -z "${DEPLOY_HOST:-}" || -z "${DEPLOY_USER:-}" || -z "${DEPLOY_PATH:-}" ]]; then
    echo "Set DEPLOY_HOST, DEPLOY_USER, and DEPLOY_PATH for remote sync (or deploy/.env.deploy.db)." >&2
    exit 2
  fi
fi
