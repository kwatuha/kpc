...
PolyMan = function(config){
	...
	this.create = function(){
		...
		if(CURRENT_ELECTION.location=="country" && !$this.code){
			url = ngAppSettings.gisApi+"county/?token="+api_token; //configure the url that will fetch shapes data from the API.
		}
		else if(CURRENT_ELECTION.location=="ward" || $this.type=="ward"){
			url = ngAppSettings.gisApi+"ward/"+$this.code+"/?token="+api_token;
		}
		else{
			url = ngAppSettings.gisApi+""+childParentMappings.down[$this.type]+"/?token="+api_token+"&"+$this.type+"="+$this.code;
		}
	...
	this.draw = function(links){ //Draw the shapes from the API to the Map
		...
		$.each(links, function(l, lnk){
            lnk.lnk=lnk.lnk.replace('http://api.iebc.or.ke/',ngAppSettings.gisApi);
			$.ajax({
				url: lnk.lnk,
				dataType:"json",
				beforeSend: function(){
